...
def train_luminar_cv(args: argparse.Namespace):
    import datasets
    import evaluate
    import numpy as np
    ...
    def compute_scores(preds: NDArray, labels: NDArray, suffix=""):
        call_uuid = str(uuid4())
        acc = evaluate.load("accuracy", experiment_id=call_uuid)
        f1 = evaluate.load("f1", experiment_id=call_uuid)
        roc_auc = evaluate.load("roc_auc", experiment_id=call_uuid)
    ...
    experiment_ulid = ulid()
    training_args = TrainingArguments(
        output_dir=f"./logs/{datset_config_name}/{experiment_ulid}",
    ...
    print("Preparing Cross Validation Datasets")
    ...
    for cv_idx, datasets_matched in enumerate(cv_datasets_matched):
        print(f"Cross Validation Run {cv_idx + 1} / {len(cv_datasets_matched)}")
        ...
        print("Saving Model")
        path_infix = f"cross_validation_{config['max_epochs']}_{args.run_ulid}"
        path_suffix = f"cv_run_{cv_idx}"
        path = save_model(
            trainer,
            config,
            infix=path_infix,
    ...
    print(f"Finished CV run {experiment_ulid} for {args.domain} on {args.agent}")

    return config | {"experiment_ulid": experiment_ulid}
...
if __name__ == "__main__":
    ...
    for domain in DOMAINS:
        ...
        references.append(train_luminar_cv.remote(args))

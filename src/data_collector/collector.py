...
class Collector(ABC):
    ...
    def write_meta_file(
        self, total_collected, date, total_synthesized=0, total_feature_extracted=0
    ):
        self.meta = {
    ...
    def get_synthesized_count(self):
        if self.synthesized_items_dfs is None:
            raise BaseException(
                "The collector hasn't synthesized yet - can't get the count. Call 'collect()' first."
            )
    ...
    def get_count(self):
        if self.meta is None:
            raise BaseException(
                "The collector hasn't collected yet - can't get the count. Call 'collect()' first."
            )
        ...
        if self.synthetization_prompt is None:
            ...
            if not os.path.exists(prompt_path):
                raise FileNotFoundError(
                    f"Couldn't find prompt file under '{prompt_path}'."
                )
            ...
            for key, value in params.items():
                placeholder = (
                    f"[{key.upper()}]"  # Match placeholders like [PLACEHOLDER]
                )

...
parser.add_argument(
    "--collectors",
    nargs="+",
    help="Pass in the collectors of this instance. Only works if --script-mode=True",
)
parser.add_argument(
    "--collect", action="store_true", help="Determine if we collect a dataset."
)
parser.add_argument(
    "--synth", action="store_true", help="Determine if we synthesize a dataset."
)
...
parser.add_argument(
    ...
    type=int,
    default=100,
    help="Determine how many items we synthesize per collector.",
)
parser.add_argument(
    "--skip",
    type=int,
    default=0,
    help="Determine how many items we skip per collector.",
)
parser.add_argument(
    "--batch_size",
    type=int,
...
parser.add_argument(
    "--featured", action="store_true", help="Determine if we feature_exract a dataset."
)
parser.add_argument(
    "--causal_llms", help="Pass in the CausalLLMs to generate the features."
)

parser.add_argument(
    "--force", action="store_true", help="Determine if we force a synthesization."
)
...
agents = [
    data_collector.agents.openai_agent.OpenAIAgent(
        name="gpt-4o-mini", api_key=os.getenv("OPENAI_API_KEY")
    )
]
...
if CONFIG.SYNTHESIZE:
    ...
    for coll in tqdm(collection, desc="Collectors", leave=False):
        ...
        collector_progress = tqdm(
            items_dfs,
            desc=f"Processing Collector Chunks of: {coll.get_folder_path().upper()}",
            leave=False,
        )
        ...
        for in_df in collector_progress:
            ...
            stored_path = os.path.join(
                coll.get_synthesized_path(), f"items_{df_count}.json"
            )
            ...
            for index, row in tqdm(
                ...
                if (
                    CONFIG.SKIP_PER_COLLECTOR > 0
                    and items_count <= CONFIG.SKIP_PER_COLLECTOR
                ):
                    items_count += 1
                    continue
                ...
                for agent in agents:
                    try:
                        item.synthetization.append(
                            {
                                "agent": agent.name,
                                "synth_obj": agent.synthesize_collected_item(
                                    item, coll
                                ),
                            }
                        )
                        ...
                        print(
                            f"There was an error with collector {coll.get_folder_path()} from source df chunk {df_count}"
                        )
                        print(ex)
                items_count += 1
...
if CONFIG.EXTRACT_FEATURES:
    for coll in tqdm(collection, desc="Collectors", leave=False):
        ...
        print(
            f"Since force is {CONFIG.FORCE_FEATURED}, items count starts at: {str(possible_skip)}"
        )
        ...
        for index, row in synth_items_df.iterrows():
                ...
                if item.synthetization is not None:
                    ...
                    for agent_obj in item.synthetization:
                        ...
                        if chunks is not None:
                            chunks["feature_space"] = extract_features(
                                chunks["synth_text"]
                            )
                        if fulltext is not None:
                            fulltext["feature_space"] = extract_features(
                                fulltext["synth_text"]
                            )
            ...
            if len(featured_items) >= CONFIG.BATCH_SIZE:
                stored_path = os.path.join(
                    coll.get_feature_space_path(), f"items_{items_count}.json"
                )

...
class Agent(ABC):
    ...
    def synthesize_chunks(self, item : CollectedItem, collector:Collector):
        ...
        synth_obj['seed'] = int(hashlib.sha256(unique_identifier.encode()).hexdigest(), 16) % (2**32)
        ...
        synth_obj['amount_chunks_to_replace'] = random.randint(1, max(1, synth_obj['total_chunks'] // 2))
        synth_obj['chunks_replaced_percentage'] = round(
            100 / synth_obj['total_chunks'] * synth_obj['amount_chunks_to_replace'],
            2
        ) 

        # Select a random starting index for the consecutive block
        synth_obj['start_idx'] = random.randint(0, synth_obj['total_chunks'] - synth_obj['amount_chunks_to_replace'])
        synth_obj['end_idx'] = synth_obj['start_idx'] + synth_obj['amount_chunks_to_replace']

        before_context = item.chunks[:synth_obj['start_idx'] - 1] if synth_obj['start_idx'] > 0 else ""
        replaced_context = item.chunks[synth_obj['start_idx']:synth_obj['end_idx']]
        synth_obj['og_chunk_text_length'] = min(5000, len(' '.join(replaced_context).split(' ')))
        after_context = item.chunks[synth_obj['end_idx'] + 1:] if synth_obj['end_idx'] < synth_obj['total_chunks'] - 1 else ""

        system_prompt = collector.get_synthetization_system_prompt({
            'date': item.date,
            'length': synth_obj['og_chunk_text_length']
        })
    ...
    def synthesize_collected_item(self, 
                                  item : CollectedItem,
                                  collector: Collector, 
                                  force=False):
        
        if not force and any(getattr(synth_item, "agent", None) == self.name for synth_item in item.synthetization):
            print('Item already synthesized with agent; skipping it. Use force=True to synthesize again.')
            return None

...
class UIMAExporter:
    ...
    def __build_xmi_from_secondary_db_item(self, item, authors):
        try:
            ...
            cas.add(DocumentAnnotation(author=authors))
            cas.add(
                UceDynamicMetadata(
                    key="date",
                    value=item["date"],
                    valueType="Enum",
                    comment="The date this text was released within the dataset (it's domain).",
                )
            )
            cas.add(
                UceDynamicMetadata(
                    ...
                    value=item["lang"],
                    valueType="Enum",
                    comment="The language of this document.",
                )
            )
            cas.add(
                UceDynamicMetadata(
            ...
            cas.add(
                UceDynamicMetadata(
                    ...
                    value=item["label"],
                    valueType="Enum",
                    comment="Either 'ai', 'human' or 'fusion'.",
                )
            )
            cas.add(
                UceDynamicMetadata(
                    ...
                    value=item["source"],
                    valueType="String",
                    comment="The original filename within the dataset this text belonged to.",
                )
            )
            cas.add(
                UceDynamicMetadata(
                    ...
                    value=item["agent"],
                    valueType="Enum",
                    comment="The model this text was created by, if it's an AI-generated text.",
                )
            )
            return cas.to_xmi(), file_name
        except Exception as ex:
            print(
                f"Couldn't parse the collected item with id {item['id']} to a XMI file."
            )
            print(ex)
            return None, None
    ...
    def __build_xmi_from_collected_item(self, item):
        try:
            ...
            cas.add(
                UceDynamicMetadata(
                    key="synthetization",
                    value=self.__sanitize_string(json.dumps(item["synthetization"])),
                    valueType="Json",
    ...
    def synch_secondary_dataset_to_uce(
        self, mongo_conn_string, uceCorpusId, dataset_name, authors
    ):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            for idx, item in enumerate(collected_items):
                xmi_string, name = self.__build_xmi_from_secondary_db_item(
                    item, authors
                )
                ...
                futures.append(
                    executor.submit(
                        self.__upload_xmi_to_uce, xmi_string, name, uceCorpusId
                    )
                )
...
if __name__ == "__main__":
    ...
    exporter.synch_secondary_dataset_to_uce(

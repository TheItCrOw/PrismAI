...
class OpenLegalDataCollector(Collector):
    ...
    def collect(self, force=False):
        ...
        if not force and os.path.exists(meta):
            self.read_meta_file()
            print(
                f"{self.meta['total_collected']} legal articles were already collected at {self.meta['collected_at']}, hence we skip a redundant collection."
            )
        ...
        for i in range(1, pages + 1):
            try:
                ...
                for article in data["results"]:
                    ...
                    if total_counter % 100 == 0 and total_counter != 0:
                        ...
                        df.to_json(
                            os.path.join(
                                output, f"articles_{total_counter // 100}.json"
                            )
                        )
                ...
                print(
                    "Exception thrown while fetching legal endpoints, probably request timeout."
                )
                print(ex)
                ex_catched += 1
            ...
            if ex_catched > 10:
                print(
                    "Too many exceptions caught, we stop this collection and store the current data."
                )
                break

...
warnings.filterwarnings(
    "ignore", message="Not mapping external offset", module="cassis.cas"
)
...
class GutenbergCollector(Collector):
    ...
    def process_xmi_content(self, xmi_content, typesystem):
        ...
        chunks = []
        for paragraph in cas.select(
            "de.tudarmstadt.ukp.dkpro.core.api.structure.type.Paragraph"
        ):
    ...
    def collect(self, force=False):
        ...
        if not force and os.path.exists(meta):
            self.read_meta_file()
            print(
                f"{self.meta['total_collected']} books were already collected at {self.meta['collected_at']}, hence we skip a redundant collection."
            )
                    ...
                    if file.endswith(".bz2"):
                        ...
                        try:
                            with bz2.open(file_path, "rt") as bz2_file:
                                ...
                                xmi_content = xmi_content.replace(
                                    '<cas:NULL id="0"/>', '<cas:NULL xmi:id="0"/>'
                                )
                                title, chunks = self.process_xmi_content(
                                    xmi_content, typesystem
                                )
                                ...
                                if total_counter % 50 == 0 and total_counter != 0:
                                    ...
                                    df.to_json(
                                        os.path.join(
                                            output, f"books_{total_counter // 100}.json"
                                        )
                                    )
                                    print(
                                        f"Books collected: {total_counter}/{max_books}"
                                    )
                                    items = []

...
class ReligionCollector(Collector):
    ...
    def collect(self, force=False):
        ...
        if not force and os.path.exists(meta):
            self.read_meta_file()
            print(
                f"{self.meta['total_collected']} chapters were already collected at {self.meta['collected_at']}, hence we skip a redundant collection."
            )
        ...
        for index, row in bibles_df.iterrows():
            ...
            if total_counter % 300 == 0 and total_counter != 0:
                ...
                df.to_json(
                    os.path.join(output, f"chapters_{total_counter // 300}.json")
                )

...
class SpiegelAPI:
    def scrape_article_html(self, article_url):
        ...
        settings = json.loads(
            doc.xpath('string(//script[@type="application/settings+json"]/text())')
        )
        ...
        return {
            ...
            "breadcrumbs": [
                ...
                for breadcrumb in ld_by_type["BreadcrumbList"]["itemListElement"]
            ],
        }
...
class SpiegelCollector(Collector):
    ...
    def collect(self, force=False):
        ...
        if not force and os.path.exists(meta):
            self.read_meta_file()
            print(
                f"{self.meta['total_collected']} articles were already collected at {self.meta['collected_at']}, hence we skip a redundant collection."
            )

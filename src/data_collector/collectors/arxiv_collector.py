...
class ArxivCollector(Collector):
    ...
    def clean_extracted_text(self, text):
        ...
        allowed_chars += ''.join(chr(i) for i in range(0xA0, 0xD7FF))  # Latin-1 Supplement and more
    ...
    def collect(self, force=False):
        ...
        if not force and os.path.exists(meta):
            self.read_meta_file()
            print(f'{self.meta['total_collected']} papers were already collected at {self.meta['collected_at']}, hence we skip a redundant collection.')
        ...
        for name in os.listdir(input):
            ...
            if counter % 100 == 0 and counter != 0:
                ...
                df.to_json(os.path.join(output, f"collected_papers_{counter // 100}.json"))
                items = []

...
class MongoDBConnection:
    ...
    def insert_collected_item(self, item: CollectedItem):
        ...
        self.collected_items_coll.update_one(
            {"id": item.id}, {"$setOnInsert": item.__dict__}, upsert=True
        )
    ...
    def get_collected_items_by_domain(self, domain, batch_size=100000, skip=0):
        ...
        try:
            with client.start_session() as session:
                cursor = (
                    client.get_database("prismai")
                    .get_collection("collected_items")
                    .find({"domain": domain}, no_cursor_timeout=True, session=session)
                    .sort([("_id", -1)])
                    .skip(skip)
                )
        ...
        try:
            with client.start_session() as session:
                cursor = (
                    client.get_database("prismai")
                    .get_collection("collected_items")
                    .find({"text": text}, no_cursor_timeout=True, session=session)
                    .sort([("_id", -1)])
                    .skip(skip)
                )
        ...
        try:
            with client.start_session() as session:
                cursor = (
                    client.get_database("prismai")
                    .get_collection("collected_items")
                    .find(no_cursor_timeout=True, session=session)
                    .sort([("_id", 1)])
                    .skip(skip)
                )

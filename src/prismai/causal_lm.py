...
class CausalLM:
    ...
    def generate(self, context, max_length=256):
        ...
        if input_ids.size(1) > max_input_length:
            print(
                f"Input sequence is too long. Truncating to fit within {max_input_length} tokens."
            )
    ...
    def generate_k_with_probs(
        ...
        steps = []
        decoder_strategy = DecodingStrategy(
            tokenizer=self.tokenizer, temp=temp, k=k, p=p
        )
        ...
        for step in range(max_length):
            ...
            if decoding_strategy == "beam_search":
                ...
                if target_idx is None:
                    current_ids = torch.cat(
                        [
                            current_ids.cpu(),
                            torch.tensor([seq[-1] for seq, _ in beam_search_sequences])
                            .unsqueeze(0)
                            .cpu(),
                        ],
                        dim=-1,
            ...
            if (
                ...
                and len(target_idx) > 0
                and len(target_idx[0]) - 1 >= step
            ):
        ...
        return {
            "generated_text": self.tokenizer.decode(
                current_ids[0, start_length:], skip_special_tokens=True
            ),
            "steps": steps,
        }

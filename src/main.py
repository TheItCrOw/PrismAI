    ...
    for essay in filtered_essays:
        ...
        for i, sentence in enumerate(sentences):
            if (i + 1) % frequence == 0:
                ...
                output = model.generate_k_with_probs(input_text, 32)

                generated = output["generated_text"]
                # We only want the first sentence from it
                llm_doc = nlp(generated)
                processed_essay.append(
                    f"[AI_FILLED]{list(llm_doc.sents)[0].text}[AI_FILLED]"
                )
...
def generate_data(label, amount, ensemble):
    ...
    with open(
        f"outputs/generator_{label}_{ensemble[0].replace('/', '_')}_{amount}_outputs.json",
        "w",
        encoding="utf-8",
    ) as f:
